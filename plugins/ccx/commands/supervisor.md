...
2. **Write the brief** at `REPO_ROOT/.ccx/tasks/<TASK.id>.md` using the template in §P2.1. Frontmatter fields come from the BOARD row; body placeholders reference `DIRECTION_TEXT` and any `TASK.notes`. **Ordering note (M8b):** the preserve-on-overwrite logic below runs BEFORE any write — substeps `a`–`d2` parse the existing brief's frontmatter for `loop_flags` (and validate / reject if invalid), and only after that succeeds does substep `e` overwrite the file with the regenerated content. Sequencing the write before the parse would erase the human-pre-seeded `loop_flags` from disk and dispatch the worker without `--duet` / `--codex-first`, breaking the M8b first-dispatch opt-in. Treat the "write the brief" header above as describing the overall step's effect; the actual on-disk write happens in substep `e`, after the preserve / validate phase.
   ...
   **M8b — `loop_flags` preserve-on-overwrite** (§17.3, SSOT for the algorithm below). If `.ccx/tasks/<TASK.id>.md` already exists on disk (a human pre-seeded duet brief, OR a supervisor-authored brief from a previous attempt that has not yet been regenerated this dispatch), the supervisor MUST preserve its `loop_flags` frontmatter entry through the regeneration. All OTHER frontmatter fields and all body sections are BOARD-sourced as before — `loop_flags` is the only field the rewrite carries through, so a stale brief cannot mask a BOARD edit. **Substeps a–d2 MUST run before any write to `.ccx/tasks/<TASK.id>.md`; substep e is the only write-to-disk site:**
   ...
   c. **Validate every entry in `loop_flags` against the M8b allowlist** before regeneration. Per §17.3, the only forwardable boolean flags are `--duet` and `--codex-first`, matched as whole tokens against `^--[a-z][a-z0-9-]+$` — no `=`, no whitespace, no value suffix. Concretely, every entry MUST be a string equal to one of `"--duet"` or `"--codex-first"`; reject anything else (including `"--duet=true"`, `"--worktree"`, `"--loops"`, `"--commit"`, `"--chat"`, leading/trailing whitespace, or non-string entries). On any rejection:
      - Capture the offending entry verbatim (first 80 chars, single-line). Print: `loop_flags[<n>] '<token>' is not on the supervisor's forwardable allowlist (M8b allows: --duet, --codex-first as boolean tokens). Edit .ccx/tasks/<TASK.id>.md to remove or correct it.`
...
3. **Commit the brief alone** on the integration branch (so the worker worktree, which forks from the current `HEAD` in step 3a below, contains the file for the 4KB-escape-hatch variant of the dispatch prompt). **M8b — "write → stage → probe → commit-or-skip"** per §17.3, replacing the unconditional commit that pre-M8b ran here. The ordering is load-bearing: probing before staging would always see an empty cached diff for the regenerated content and skip the commit even when BOARD-derived fields legitimately changed, leaving the worker forked from an old `HEAD` whose brief did not reflect the regeneration:

   1. Write the regenerated brief to `.ccx/tasks/<TASK.id>.md` (overwrites the on-disk file; already done in step 2e above for the regeneration, but if any prior step bypassed it the write MUST happen here).
   2. `git add -- .ccx/tasks/<TASK.id>.md` (stage the regenerated content).
   3. Probe with `git diff --cached --quiet -- .ccx/tasks/<TASK.id>.md`.
...
7. **Update `RUNNING[<task_id>]` in place.** First, capture the pre-update tier into a local variable: `old_tier = meta.tier` (read BEFORE any mutation; step 8's audit branch keys off `old_tier vs next_tier` and would always see them as equal if step 8 ran after the tier was already updated). Then overwrite `shell_id`, `log_path` (same path — `.ccx/workers/<task_id>.log`), `started_at` (the new pre-spawn `STARTED_AT`), `attempts: meta.attempts + 1`, and `tier: next_tier`. Keep `worktree_path`, `branch`, `scope_include`, `last_signal` (already updated in the entry bookkeeping), and `loop_flags` (M8b — preserved across re-dispatches so the §P2.2 suffix builder keeps emitting `--duet` / `--codex-first` for every retry) unchanged. Do NOT remove or re-add `<task_id>` to `DISPATCHED` — same task id, same ownership. Advancing `started_at` is load-bearing: Step B's sub-classifier on the NEXT exit uses `at >= started_at` to reject the prior attempt's closure record; leaving the first attempt's `started_at` in place would readmit the prior stuck/cap closure into the scoped set and re-trigger recovery redundantly.
...
- **Blocked** (`<count>`): list `T-<id>` — `<exit_status>` — log path (`.ccx/workers/T-<id>.log`) — `attempts=<N>` — `final-tier=<alias>/<effort>` (the `attempts=` and `final-tier=` suffixes are only printed when `attempts > 1`; first-attempt blocks skip them). Blocked reasons: `stale-artifact | spawn-error | merge-conflict | merge-aborted | merge-commit-failed | no-commit | error | attempts-exhausted | stuck-aborted | stuck-recovery-failed | stuck-cleanup-failed | loop-flags-rejected`. M-specific reasons:
  ...
  - `loop-flags-rejected` (M8b, §17.3 / §17.9): the brief's `loop_flags` frontmatter carried an entry outside the supervisor's forwardable allowlist (anything other than the whole tokens `--duet` / `--codex-first`), or `--codex-first` was present without `--duet`. The task never spawned a worker — the rejection fires inside Step A step 2c before brief commit / worktree creation. Edit `.ccx/tasks/T-<id>.md` to remove or correct the offending entry, then flip BOARD status to `pending` and re-run. This is a deterministic, bounded rejection — the session-close classifier (P0.5 step 7 rule 5) treats it as a `completed`-flavored exit, NOT stuck.

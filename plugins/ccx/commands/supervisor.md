...
2. **Dogfood short-circuit.** `git config --get --type=bool ccx.dogfood` returning `true` → `STATE_DIR = REPO_ROOT/.ccx/`. This is the ccx-loop repo's mode and the ONLY mode in which a `.ccx/` directory legitimately appears in the working tree. The flag must be set explicitly per-repo; there is no global override and no auto-detection from repo name. The repo may additionally commit a `.ccx-config` file as dogfood metadata; `ccx verify` (T-6) treats `.ccx-config` as the single permitted dogfood-only exception and refuses it in non-dogfood checkouts.
...
**In-repo `STATE_DIR` requires explicit dogfood opt-in.** ANY resolved `STATE_DIR` that lies inside `REPO_ROOT` REQUIRES `git config --get --type=bool ccx.dogfood` to return `true`. The check fires before any directory creation or the one-line stderr announcement. Two rejection cases:
...
1a. **Resolve `STATE_DIR` and `IS_DOGFOOD`** per the "State path resolver" section above. Run the resolver exactly once at this step and cache TWO run-scope variables:
   ...
   - `IS_DOGFOOD` — boolean. **True ONLY when BOTH:** (a) `STATE_DIR` equals `REPO_ROOT/.ccx/` exactly (with trailing-slash normalization), AND (b) `git config --get --type=bool ccx.dogfood` returns `true`. Concretely:
     ```bash
   ...
   **M9 T-2 — resolve `IS_PARANOID`** in the same block (the worktree path resolver above reads it once per task and the cached value is the sole determinant of paranoid-mode behaviour for the rest of the run):
     ```bash
     IS_PARANOID="$(git config --get --type=bool ccx.paranoid 2>/dev/null || echo false)"
     ```

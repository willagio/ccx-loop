...
Defaults (`--worker-loops 3`, `--worker-mode duet`, `--start-tier auto`) are chosen so that running `/ccx:supervisor` with no flags keeps the M8b duet contract while M10 conductor mode incubates.

...
- **M9 brief-read contract (always-on, runs before any phase reads a brief).** When the dispatch prompt's `<task_brief path="…">` attribute is an absolute path, the path is allowed for `Read` ONLY when ALL of the following hold; on any failure, refuse the read and STOP via `chat_close({status: "aborted"})`:
  ...
  4. If `$CCX_TASK_BRIEF_PATH` is unset or empty, the M9 exception is OFF — the worker has no trusted source for the brief path and falls back to the pre-M9 rule: only paths inside the current worktree are readable. This is the direct-invocation case (`/ccx:loop` run from a shell, no supervisor).
...
Runs after the staging set is computed and the draft message is written, but BEFORE `git commit` fires. The pipeline has three steps; the first two are gated on `ccx.dogfood`, the third is independent.

**Mode resolution.** Read once at the top of the pipeline:

- `IS_DOGFOOD = git config --local --get --type=bool ccx.dogfood` (treat absent/error as `false`). The `--local` scope is load-bearing — `docs/supervisor-design.md` §18.4 establishes that no global or system defaults are honoured for any ccx.* key, and a bare `git config --get` would silently activate dogfood mode (skipping T-3 hygiene and letting `T-<id>:`/`supervisor:` subjects leak into customer commits) for any customer repo on a machine where the operator set `ccx.dogfood = true` at global scope. The flag is the single dogfood escape hatch all M9 customer-mode behaviors consult; see §18.2.8 (T-5) for the full behavior matrix and `/ccx:where` to inspect the path it controls. **Mirror drift caveat:** `plugins/ccx/commands/forever.md` carries the same commit-hygiene subsection verbatim per §18.2.6 and is OUT of T-5's scope; until a follow-up task flips it too, `forever.md`'s `IS_DOGFOOD` read is still bare `--get` and inherits global. The drift is documented in §18.2.8 as a known follow-up; the correctness-vs-lockstep trade-off favours correctness here (closing the global-flag leak in `/ccx:loop` is the headline T-5 invariant; the bare-read remnant in `/ccx:forever` is a residual leak the follow-up will close).
- `WANT_TRAILER = git config --local --get --type=bool ccx.commit.trailer` (treat absent/error as `false`). The `--local` matches the dogfood gate above and the rest of the M9 ccx.* contract — a bare `--get` would inherit a global `ccx.commit.trailer = true` and append `Ccx-Task: T-<id>` trailers to customer commits whose operators never opted in. The mirror in `forever.md` is still bare; closing that drift is part of the same follow-up noted next to the `IS_DOGFOOD` read above.
- `TASK_ID = $CCX_TASK_ID` env var (set by `/ccx:supervisor` Step A step 4 alongside `$CCX_TASK_BRIEF_PATH`). Falls back to the `<task_brief id="...">` attribute in the dispatch prompt when the env var is unset; falls back to `null` for direct (non-supervisor) `/ccx:loop` invocations.
...
Default `WANT_TRAILER == false` produces no trailer. When `TASK_ID` is null (direct `/ccx:loop` invocation without a supervisor), the trailer step is silently a no-op — there is nothing to trail.

**Why two layers, not one.** The regex gate alone could mangle natural-language commits ("supervisor: dispatch" rewritten as "dispatch:" reads poorly); the LLM pass alone is non-deterministic and can regress. The two-layer design lets the LLM produce a clean rewrite the regex then audits, with three retries before giving up. The same regex is re-applied at merge time as defence in depth: T-4's `/ccx:supervisor` Step B step 3 squash-strategy path re-runs Step 2's regex against the worker's final commit message before the squash commit lands on integration (a hit aborts the merge with `exit_status: "leak-detected-at-merge"` — see `plugins/ccx/commands/supervisor.md`'s "Merge strategy resolver" section), and T-6's `ccx verify` will re-apply the same regex at mainline boundary as a defence-in-depth gate for any future merge primitive — see `docs/supervisor-design.md` §18.2.6 (T-3 anchor) and §18.2.7 (T-4 merge-boundary re-check).

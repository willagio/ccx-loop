...
- `WANT_TRAILER = git config --local --get --type=bool ccx.commit.trailer` (treat absent/error as `false`). The `--local` scope matches the rest of the ccx.* contract — a bare `--get` would inherit a global `ccx.commit.trailer = true` and append `Ccx-Task: T-<id>` trailers to commits whose operators never opted in.
...
**Step 2 — opt-in `Ccx-Task` trailer.** When `WANT_TRAILER == true` AND `TASK_ID` is non-null, append a parseable Git trailer to the rewritten message body. Use `git interpret-trailers --in-place --trailer "Ccx-Task: <TASK_ID>"` against the commit-message file (or `--no-divider` against stdin if no file is in flight), which:
...
- is parseable later via `git interpret-trailers --parse`.

...
```bash
CODEX_ROOT="$(find ~/.claude/plugins/marketplaces/openai-codex/plugins/codex ~/.claude/plugins/cache/openai-codex/codex -maxdepth 0 -type d 2>/dev/null | head -1)" && node "$CODEX_ROOT/scripts/codex-companion.mjs" task --write --json $CODEX_MODEL_ARGS "$DUET_CODEX_PROMPT"
```
...
`$DUET_CODEX_PROMPT` is built identically to Claude's implement prompt (style clause + brief excerpt + reject findings or approve instruction), routed to the Codex CLI instead of executed in-session. `CODEX_MODEL_ARGS` for this invocation comes from the per-cycle resolution defined in **Codex model selection per cycle** below — do NOT inline tier resolution here.
...
3. **Run the turn.**
   ...
   - **Review turn:** spawn the reviewer sub-process per "Sub-process primitives". Parse the reviewer's envelope (Codex JSON via `verdict`, Claude `VERDICT:` line plus fenced-JSON findings block). Partition findings into in-scope vs skipped using `--min-severity` and `--min-confidence`. Append the turn's record to `cycle_log` (bounded to the last `N = 3` records).
     ...
     - **Stuck-judgment check:** when `cycle_log` is full AND the most recent record's `verdict == "reject"`, invoke the LLM judgment described in "Convergence and stuck detection". If the judgment returns yes-stuck with high confidence, write the `decision: "stuck-judgment"` audit record, set `conductor_exit_status = "stuck"`, and break out of the per-turn loop. Phase 4's `finally`-block emits the `M8B_STUCK_SIDE` log token IMMEDIATELY before `chat_close({status: "stuck"})` fires, as defined in "Convergence and stuck detection".
4. **Cycle accounting.** Every time a review turn completes, `cycles_used += 1` (one implement + one review = one cycle, matching Phase 2-Duet's `cycles_used` semantics). If `cycles_used >= --loops N` AND termination has not fired, write the `decision: "exit"` audit record, set `conductor_exit_status = "budget-exhausted"`, and break out of the per-turn loop. Do NOT call `chat_close` here — Phase 4 owns the single close.
...
- `decision: "exit"` — once at end-of-run, includes `{conductor_exit_status, cycles_used, approval_counter, last_stuck_implementer_side}`.
